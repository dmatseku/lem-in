...
{
	data->first_path_first_part = index;
...
static uint32_t skip_death_zone(t_path *path,
					uint32_t start_index, t_node_data *nodes_data)
{
	uint32_t owner;
	uint32_t res;

	owner = nodes_data[path->path[start_index]].owner;
	res = 1;
	start_index++;
...
void	swap_paths(t_path *path_1, t_path *path_2, uint32_t start_index,
